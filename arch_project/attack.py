...
def sam_encoder(image):
    ...
    masks = masks_update(masks, iou_thr=0.8, score_thr=0.7, inner_thr=0.6)
...
def generate_noise(indx, image_orj, seg_images, seg_image_masks, seg_map, learning_rate= 0.1, it=70):
    ...
    for i, img in tqdm(enumerate(seg_images)):
        ...
        #to_pil_image(img_masked).save(save_folder + "/" + str(i) +  "_" + data_list[indx])
        if pred > best_pred:
            best_pred = pred
            best_mask = seg_map[i]
            best_image_id = i
            best_image = img_masked
    ...
    seg_width = np.sum(best_mask["segmentation"][int(best_mask["segmentation"].shape[0] / 2)])
    multiplier = int(seg_width / 36)
    subregion_width = 11 * multiplier
    subregion_height = 17 * multiplier
    
    # Calculate the top-left corner of the subregion
    start_row = (seg_map[best_image_id]["segmentation"].shape[0] - subregion_height) // 2
    start_col = (seg_map[best_image_id]["segmentation"].shape[1] - subregion_width) // 2

    # Set the middle area of 110x170 to True
    seg_map[best_image_id]["segmentation"].fill(False)
    seg_map[best_image_id]["segmentation"][start_row:start_row + subregion_height, start_col:start_col + subregion_width] = True
    best_mask = seg_map[best_image_id]
    ...
    for step in range(it):
        ...
        loss = criterion(outputs, torch.tensor([target_label], device="cuda:1"))
        model.zero_grad()
        loss.backward()
        ...
        if probs[0,true_label] == 0.0 and loss.item() > 20.00 and probs[0,target_label] == 1.0:
            break
    ...
    noise = merged_img[:, start_row:start_row + subregion_height, start_col:start_col + subregion_width]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_name', type=str, default="door")
    parser.add_argument('--true_label_name', type=str, default="door")
    ...
    dataset_path = os.path.join("arch_project/data/" + args.dataset_name)
    ...
    for i, data_path in tqdm(enumerate(data_list_raw), leave=False):
        ...
        if h > args.resolution or w > args.resolution :
            print("Resolution is too high, resizing ..") 
