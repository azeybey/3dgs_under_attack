...
def generate_noise(indx, image_orj, seg_images, seg_image_masks, seg_map, learning_rate= 0.02, it=30):
    ...
    for i, img in tqdm(enumerate(seg_images)):
        ...
        pred, im = predict(to_pil_image(img_masked))
        if pred > best_pred:
            best_pred = pred
            best_mask = seg_map[i]
            best_image_id = i
    ...
    if best_pred < 0.05:
        print(f"Confidence:{best_pred} is below threashold, skipping...")
        image_orj_ = image_orj[[2, 1, 0], :, :]
        image_orj_ = to_pil_image(image_orj_)
        image_orj_.save(save_folder + "/" + data_list[indx])
        return
    
    image = to_pil_image(best_image)
    image.save(save_folder + "/best_segment_" + data_list[indx])
    image = transform(image).unsqueeze(0).to("cuda:1")
    ...
    seg_image_mask = cv2.resize(seg_image_masks[best_image_id].astype(np.uint8), (224, 224), interpolation=cv2.INTER_NEAREST).astype(bool)
    ...
    predict(image)
    image.save(save_folder + "/raw_" + data_list[indx])

    resized_image = image.resize((seg_images[best_image_id].shape[0], seg_images[best_image_id].shape[1]), Image.BICUBIC)
    print("Resized Image Prediction : ")
    predict(resized_image)
    resized_image.save(save_folder + "/resized_" + data_list[indx])
    

    merged_img = merge(image_orj, resized_image, best_mask, seg_image_masks[best_image_id])
    
    print("Orijinal Image Prediction : ")
    orj_res, orj_im = predict(to_pil_image(image_orj))

    orj_im = to_pil_image(unnormalize(orj_im).permute(2,0,1))
    orj_im.save(save_folder + "/orj_im_" + data_list[indx])

    print("Noisy Image Prediction : ")
    merged_res, merged_im = predict(merged_img)
    merged_im = to_pil_image(unnormalize(merged_im).permute(2,0,1))
    merged_im.save(save_folder + "/merged_im_" + data_list[indx])
...
def merge(image_orj, resized_image, seg_map, seg_image_mask):

    resized_image_tensor = torch.from_numpy(np.array(resized_image)).permute(2,0,1)  # From HWC to CHW and to float
    masked_image_tensor = resized_image_tensor.clone().detach().cpu().numpy() * seg_image_mask[None, :, :]
    
    
    image_orj_rgb = image_orj[[2, 1, 0], :, :]
    inverse_image_orj = image_orj_rgb * np.logical_not(seg_map["segmentation"][None, :, :])

    x, y, w, h = seg_map['bbox']

    inverse_image_orj[:, y:y+h, x:x+w] += masked_image_tensor
